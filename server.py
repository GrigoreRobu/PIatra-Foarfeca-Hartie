...
def game_start(client_socket):
    ...
    while player_score < 2 and computer_score < 2:
        client_socket.send("Alege varianta (P/H/F): ".encode())
        player_choice = client_socket.recv(1024).decode().strip().upper()

        if player_choice not in ['P', 'H', 'F']:
            client_socket.send("Nu există o astfel de variantă. Verificați și introduceți din nou.\n".encode())
            continue
    ...
    if player_score > computer_score:
        client_socket.send(f"Ai câștigat din {player_score} încercări!\n".encode())
...
print('Serverul TCP este pregătit de lucru și ascultă pe adresa:', server_socket.getsockname())
...
while True:
    ...
    while True:
        client_socket.send("Scrie START pentru a începe jocul de Piatra Foarfeca Hartie sau Q pentru a ieși.\n".encode())
        ...
        if start_command == "START":
            ...
            client_socket.send("Comandă necunoscută. Vă rog să introduceți START pentru a începe jocul sau Q pentru a ieși.\n".encode())
    client_socket.close()
